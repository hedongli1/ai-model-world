...
> | 3 | **视觉二次开发** | 新增「星野暖金」主题层（v3，见下） |
>
> **业务逻辑、数据管线、组件结构零改动。**

> ### 🎨 二次开发：「星野暖金」主题层（2026-09-18，v3）
>
...
> | 版本 | 定位 | 问题 |
> |---|---|---|
> | v1 深海霓虹 | 青绿色系 | **配色缺陷**：天空渐变末端被改成冷青、中段却保留暖红，色温中途拧转成「彩虹跳色」 |
> | v2 深海光柱 | 修渐变 | 用户体验后认为青色霓虹描边太扎眼（「流线型」），且星点太少背景单调 |
> | **v3 星野暖金（当前）** | 冷底+暖金 | ✅ 见下 |
>
> **当前主题（v3）：**
> - **渐变**：近黑 `#080a12` → 墨蓝 `#10182a` → 靛蓝 `#1e2e4a` → 暖赭 `#a8703c` → 暖金 `#e8b45c`
>   （色温从左冷到右暖单向推进，保留「黎明」语义）
> - **强调色**：主色 / 皇冠 = 暖金 `#e8b84a`；建筑归为冷蓝灰系贴合夜色
> - **流线型元素已移除**（SectionFrame 外发光 + 扫光条；保留边框/角标/色晕的「仪器卡口」语义）
> - **新增 CSS 星野**：`body::before` 三层 180 个星点（冷白/暖米/蓝白），固定种子确定性，
>   零 JS、零 canvas，SSR/CSR 恒一致，`prefers-reduced-motion` 时隐藏
> - **实现方式**：Tailwind v4 `@layer theme` 内以 `:root,:host` 选择器重定义颜色变量
> - **回退方式**：删除该注释块（从「二次开发主题层」到「回退说明结束」）即完全恢复上游原版视觉
> - **踩坑记录**：① Tailwind v4 覆盖必须带 `:root,:host`，裸写变量是孤儿声明静默丢弃；
>   ② 改渐变时要看色温在整条上的连贯性，不能只看单色好不好看
>
